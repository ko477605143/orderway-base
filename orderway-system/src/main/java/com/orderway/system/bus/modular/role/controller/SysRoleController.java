 ...
 */
package com.orderway.system.bus.modular.role.controller;
...
public class SysRoleController {
    ...
    public ResponseData page(SysRoleParam sysRoleParam) {
        return new SuccessResponseData(sysRoleService.getPage(sysRoleParam));
    }
     ...
     *
     */
    @GetMapping("/sysRole/getAllApp")
    @BusinessLog(title = "系统角色_获取所有应用下拉框", opType = LogAnnotionOpTypeEnum.DETAIL)
    public ResponseData getAllApp() {
        return new SuccessResponseData(sysRoleService.getAllApp());
    }
    ...
    @BusinessLog(title = "系统角色_查询", opType = LogAnnotionOpTypeEnum.QUERY)
    public ResponseData getPage(SysRoleParam sysRoleParam) {
        return new SuccessResponseData(sysRoleService.getPage(sysRoleParam));
    }
}
