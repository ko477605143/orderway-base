...
import com.orderway.system.bus.core.enums.DataScopeTypeEnum;
...
import com.orderway.system.bus.modular.role.enums.SysRoleExceptionEnum;
import com.orderway.system.bus.modular.role.mapper.SysRoleMapper;
import com.orderway.system.bus.modular.role.param.SysRoleParam;
import com.orderway.system.bus.modular.role.service.SysRoleDataScopeService;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
     ...
     */
    @Override
    public List<SysApp> getAllApp() {
        LambdaQueryWrapper<SysApp> qw = new LambdaQueryWrapper();
        qw.eq(SysApp::getStatus,CommonStatusEnum.ENABLE.getCode()).select(SysApp::getId,SysApp::getName,SysApp::getCode);
        List<SysApp> list = sysAppService.list(qw);
        return list;
    }
     ...
     */
    @Override
    public PageResult<Map> getPage(SysRoleParam sysRoleParam) {
        QueryWrapper<SysRole> qw = new QueryWrapper();
        if (ObjectUtil.isNotNull(sysRoleParam)) {
            //根据名称模糊查询
            if (ObjectUtil.isNotEmpty(sysRoleParam.getName())) {
                qw.like("t1.name", sysRoleParam.getName());
            }
            //根据编码模糊查询
            if (ObjectUtil.isNotEmpty(sysRoleParam.getCode())) {
                qw.like("t1.code", sysRoleParam.getCode());
            }
        }

        qw.eq("t1.status", CommonStatusEnum.ENABLE.getCode());
        return new PageResult<>(this.baseMapper.getPage(PageFactory.defaultPage(),qw));
    }

    @Override
    public List<Map<String,Object>> getAllRoleByUserId(Long userId) {
        return this.baseMapper.getAllByUserId(userId);
    }

    /**
     * 根据登入用户，获取所有的角色
     */
    public List<Dict> getAllRole(SysLoginUser sysLoginUser) {
        List<Map<String,Object>> list = CollectionUtil.newArrayList();
        // 从request中获取Token，根据Token获取用户信息
        if(AdminTypeEnum.SUPER_ADMIN.getCode().equals(sysLoginUser.getAdminType())) {
            ...
            qw.eq(SysRole::getStatus,CommonStatusEnum.ENABLE.getCode());
            list = this.listMaps();
        }else {
            // 根据用户Id 拿到当前用户的所有角色信息
            list = this.getAllRoleByUserId(sysLoginUser.getId());
        }
        // 将SysRole数据填充到Dict中
        List<Dict> dicts = CollectionUtil.newArrayList();
        list.forEach(i -> {
            Dict dict = Dict.create();
            dict.put(CommonConstant.ID, i.get("id"));
            dict.put(CommonConstant.CODE, i.get("code"));
            dict.put(CommonConstant.NAME, i.get("name"));
            dict.put("app_name", i.get("app_name"));
            dict.put("app_id", i.get("app_id"));
    ...
    @Override
    public List<Map> getRolesByCode(String code ,Long userId) {
        return this.baseMapper.getRolesByCode(code,userId);
    }

    @Override
    public List<Map> getUserByToken(Long userId, String roleCode) {
        return this.baseMapper.getUserByToken(userId, roleCode);
    }

    @Override
    public List<Map> getAllRole() {
        return this.baseMapper.getAllRole();
    }
}
