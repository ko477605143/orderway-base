 ...
 */
public class SysLoginUserFactory {
    ...
    public static void fillLoginUserInfo(SysLoginUser sysLoginUser) {
        ...
        if (ObjectUtil.isNotNull(request)) {
            ...
            // 角色信息
            List<Dict> roles = sysRoleService.getAllRole(sysLoginUser);
            sysLoginUser.setRoles(roles);
            List<Long> ids = roles.stream().map(i -> i.getLong("id")).collect(Collectors.toList());
           ...
           // 权限信息
            ...
            List<Dict> apps = sysAppService.getAllApp(ids);
            ...
            if (ObjectUtil.isEmpty(apps)) {
                ...
                sysLoginUser.setMenus(sysMenuService.getUserMenu(userId,defaultActiveAppCode));
            }
