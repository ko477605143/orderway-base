 ...
 */
package com.orderway.system.bus.modular.menu.service.impl;
...
import com.orderway.core.factory.TreeBuildFactory;
...
import com.orderway.system.bus.core.enums.MenuOpenTypeEnum;
import com.orderway.system.bus.core.enums.MenuTypeEnum;
import com.orderway.system.bus.core.enums.MenuWeightEnum;
...
import com.orderway.system.bus.modular.menu.enums.SysMenuExceptionEnum;
import com.orderway.system.bus.modular.menu.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    @Override
    public List<MenuBaseTreeNode> treeByApplication(String application) {
        List<MenuBaseTreeNode> menuTreeNodeList = CollectionUtil.newArrayList();
        LambdaQueryWrapper<SysMenu> qw = new LambdaQueryWrapper<SysMenu>();
        if (ObjectUtil.isNotEmpty(application)) {
            qw.eq(SysMenu::getApplication, application);
        }
        // 取出应用下的所有菜单
//        LambdaQueryWrapper<SysMenu> qw = new LambdaQueryWrapper<SysMenu>()
        qw.eq(SysMenu::getStatus, CommonStatusEnum.ENABLE.getCode())
                .in(SysMenu::getType,
                        CollectionUtil.newArrayList(MenuTypeEnum.DIR.getCode(),
                                MenuTypeEnum.MENU.getCode(),
                                MenuTypeEnum.BTN.getCode()));
        this.list(qw).forEach(sysMenu -> {
            MenuBaseTreeNode menuTreeNode = new MenuBaseTreeNode();
            menuTreeNode.setId(sysMenu.getId());
            menuTreeNode.setParentId(sysMenu.getPid());
            menuTreeNode.setValue(String.valueOf(sysMenu.getId()));
     ...
     */
    @Override
    public List<LoginMenuTreeNode> getAllMenuByApp(Long userId, String appCode) {
        ...
        // 将SysMenu格式菜单转换为LoginMenuTreeNode菜单返回
        return this.convertSysMenuToLoginMenu(list);
    }

    /**
     * 根据用户 和code 查询对应的所有菜单
     *
     * @param userId 用户id
     */
    @Override
    public List<Map> getOneLevelMenusByUserId(Long userId) {
        return this.baseMapper.getOneLevelMenusByUserId(userId, CommonStatusEnum.ENABLE.getCode(), MenuTypeEnum.BTN.getCode());
    }
    ...
    public List<SysMenu> getMenusByPid(Long menuId) {
        ...
        // 菜单的pid = 传入的menuId 并且 type 不是按钮
        qw.eq(SysMenu::getPid, menuId).ne(SysMenu::getType, MenuTypeEnum.BTN.getCode())
                //状态是正常
                .eq(SysMenu::getStatus, CommonStatusEnum.ENABLE.getCode())
                // 按照sort升序排列
                .orderByAsc(SysMenu::getSort);
    ...
    @Override
    public List<LoginMenuTreeNode> getUserMenu(Long userId, String menuCode) {
        //如果是超级管理员则展示所有系统权重菜单，不能展示业务权重菜单
        SysUser sysUser = sysUserService.getById(userId);
        // 判断是否是超级管理员
        if (AdminTypeEnum.SUPER_ADMIN.getCode().equals(sysUser.getAdminType())) {
            // 查询权重是 系统权重
            return this.getAllMenuByApp(userId, menuCode);
        }
        List<SysMenu> userMenu = this.baseMapper.getUserMenu(sysUser.getId(), menuCode, CommonStatusEnum.ENABLE.getCode(), MenuTypeEnum.BTN.getCode());
        // 当前应用没有菜单 返回空集合
        if (ObjectUtil.isEmpty(userMenu)) {
            return CollectionUtil.newArrayList();
        }
        // 将SysMenu格式菜单转换为LoginMenuTreeNode菜单返回
        return this.convertSysMenuToLoginMenu(userMenu);
    }

    @Override
    public List<Map> getAllMenu(Long userId, Integer type) {
        return this.baseMapper.getAllMenu(userId, type);
    }
}
