...
import com.orderway.system.bus.modular.emp.mapper.SysEmpExtOrgPosMapper;
...
public class SysEmpExtOrgPosPosServiceImpl extends ServiceImpl<SysEmpExtOrgPosMapper, SysEmpExtOrgPos> implements SysEmpExtOrgPosService {
     ...
     */
    @Override
    public void addOrEditByList(List<Dict> list, Long empId) {
        // 先删除附属信息
        this.deleteEmpExtInfoByUserId(empId);
        // 添加附属信息
        if (CollectionUtil.isNotEmpty(list) && list.size() > 0) {
            list.forEach(i -> {
                        Long uuid = IDCommon.getUUID();
                        this.baseMapper.addExtList(uuid, i, empId);
                    }
            );
        }
    }
